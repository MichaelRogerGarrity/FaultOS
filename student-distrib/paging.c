...
}__attribute__ ((packed)) page_dir_entry; //1kb
...
// set the page dirctory 
    ...
    // if( i == 0){ // set the video mem 
        ...
        //     page_directory[i].pt_baddr = (uint32_t) 0x00400000 >> 12; //cr3 
        // }
    // else{ //all other page table entries
        ...
        page_table[i].a =  0;   
        page_table[i].DC1 =  0;   
        page_table[i].ps = 0;   // 4mb page table
    ...
    page_directory[0].pt_baddr = (uint32_t)(0xB8000 >> 12);

    //set up the kernel 
    page_directory[0].p = 1;
    // what else? page_directory[0].us = 0; // set to kernel permission 
    page_directory[0].pt_baddr = (uint32_t)(0x00400000 >> 12);
